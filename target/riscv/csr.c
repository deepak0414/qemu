                                    ...
                                    target_ulong val)
{
    ...
    if (riscv_cpu_mxl(env) == MXL_RV64) {
        ...
        /* If shadow stack enabling bit changes in menvcfg, flush tb */
        if ((val ^ env->menvcfg) & MENVCFG_SSE) {
            tb_flush(env_cpu(env));
        }
                                    ...
                                    target_ulong val)
{
    ...
    if (env_archcpu(env)->cfg.ext_cfi_lp &&
        get_field(env->menvcfg, MENVCFG_LPE) &&
        (env->virt_enabled ? get_field(env->henvcfg, HENVCFG_LPE) : true)) {
        mask |= SENVCFG_LPE;
    }
    ...
    /* If shadow stack enabling bit changes in senvcfg, flush tb */
    if ((val ^ env->senvcfg) & SENVCFG_SSE) {
        tb_flush(env_cpu(env));
    }
                                    ...
                                    target_ulong val)
{
    ...
    if (riscv_cpu_mxl(env) == MXL_RV64) {
        ...
        /*
         * If ss and lp available and menvcfg.CFI = 1, then apply ss and lp
         * masks for henvcfg
         */
        if (env_archcpu(env)->cfg.ext_cfi_ss &&
            get_field(env->menvcfg, MENVCFG_SSE)) {
        ...
        /* If shadow stack enabling bit changes in henvcfg, flush tb */
        if ((val ^ env->henvcfg) & HENVCFG_SSE) {
            tb_flush(env_cpu(env));
        }
    }
